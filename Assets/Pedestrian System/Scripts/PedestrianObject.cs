...
public class PedestrianObject : MonoBehaviour 
{
	...
	public  GameObject                          m_meshObject          = null;                 // you need to assign the root gameobject that holds all the rendering components of the pedestrian. This is to turn this pedestrian on and off when culling is enabled. 
	...
	void FixedUpdate()
	{
		if (PedestrianSystem.Instance && m_meshObject) 
		{
			// this is used for culling the vehicle once it is out of view.
			if (PedestrianSystem.Instance.IsInView (transform.position))
